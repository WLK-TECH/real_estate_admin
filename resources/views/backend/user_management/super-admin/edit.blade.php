        ...
        <div class="col-md-6 offset-md-3">
            ...
            <form action="{{ route('super-admin.update', $superAdmin->id) }}" method="POST">
                ...
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control" name="name" value="{{ $superAdmin->name }}" placeholder="Enter Name">
                    @error('name')
                ...
                <div class="mb-3">
                    ...
                    <small class="d-block">Old Email - {{ $superAdmin->email }}</small>
                    @error('email')
                    <span class="d-block text-danger">*{{ $message }}</span>
                    @enderror
                </div>
                <div class="mb-3">
                    ...
                    <select class="js-example-basic-multiple form-control" name="roles[]" multiple="multiple">
                        @foreach ($roles as $role)
                            @php
                                $selected = $superAdmin->roles->contains('id', $role->id) ? 'selected' : '';
                            @endphp
